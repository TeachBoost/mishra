...
// Which Module to build? If script is run with argument, use that
// Minify, debug? 
...
function buildModule(module) {
    ...
    if (fs.existsSync(modulePath + module + '/index.js')) {
        ...
        throw new Error('Module does not seem to exist. Looked for ' + modulePath + module + '/index.js');
    }
...
function copyJsFiles(callback) {
    if (config.base.js.value.length > 0) {
        ...
        config.base.js.value.forEach(function(jsFile) {
            fs.createReadStream(vendorPath+jsFile.source).pipe(fs.createWriteStream(jsPath+jsFile.target));
        });
    }
...
function lessCssFiles(callback, module) {
    ...
    var lesscProcess = childProcess.exec(command, function (error, stdout, stderr) {
        console.log(stdout);
        console.log(stderr);
        if (error !== null) {
...
function browserifyBase(paths, dependencies, outFile) {
    ...
    dependencies.map(function(dependency) {
        ...
        if (dependency.require.match('\/vendor\/')) {
            //if so, have to update the directory so it's relative to the where the script is called
...
function browserifyModule(module) {
    ...
    .bundle(function (err, data) {
        ...
        if(err) { console.log('module error'); throw err; }
        return data;
    })
